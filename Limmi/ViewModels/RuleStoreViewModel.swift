...
class RuleStoreViewModel: ObservableObject {
    ...
    /// 1. Saves beacon device if new (optional)
    /// 2. Converts and saves all blocked tokens
    /// 3. Creates the rule with all references
    /// - Parameters:
    ///   - name: Rule name
    ///   - beacon: Beacon device (optional - can be nil)
    ...
    func createComplexRule(
        ...
        // Step 1: Save beacon device if provided (if new) or use existing
        if let beacon = beacon {
            ...
            // No beacon - skip beacon saving and create rule without beacon
            logger.debug("No beacon provided, creating rule without beacon")
            
            // Step 2: Convert BlockedTokens to BlockedTokenInfo and save
            saveBlockedTokensAsBlockedTokenInfo(blockedTokens) { [weak self] result in
                switch result {
                case .success(let blockedTokenIds):
                    // Step 3: Create the rule without beacon
                    self?.createRuleWithComponents(
                        name: name,
                        beaconId: nil,
                        gpsLocation: gpsLocation,
    ...
    private func createRuleWithComponents(
        ...
        let beaconDescription = beaconId ?? "none"
        logger.debug("Creating rule with components: beacon=\(beaconDescription), tokens=\(blockedTokenIds.count)")
