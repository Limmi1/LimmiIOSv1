...
final class BlockingEngine: ObservableObject {
    ...
    private let heartbeatManager: HeartbeatProtocol?
    ...
    init(
        ...
        beaconMonitor: BeaconMonitorProtocol,
        clock: Clock,
        ruleStore: RuleStore,
        blocker: Blocker,
        configuration: Configuration = .default,
        heartbeatManager: HeartbeatProtocol? = nil,
    ...
    private func handleRulesChanged(_ rules: [Rule]) {
        logger.debug("Rules changed: \(rules.count) active rules")
        ...
        syncActiveRuleTokensToSharedStorage(rules: rules)
    ...
    private func updateMonitoredBeacons() {
    ...
    private func evaluateBlockingRules() {
        ...
        // Apply blocking decisions based on the evaluation result
        blocker.applyRuleBasedBlocking(
            for: Array(allBlockedAppIds),
            using: { appId in evaluationResult.blockedAppIds.contains(appId) }
        )
        ...
        logger.debug("[\(evaluationId)] Evaluating \(rules.count) rules at location \(location.coordinate) with beacons[\(beaconRegionStatus)]: \(evaluationResult.blockedAppIds.count)/\(allBlockedAppIds.count) apps blocked")
    }
