...
final class RegionMonitoringLocationProvider: NSObject, LocationProvider {
    ...
    override init() {
        ...
        super.init()
        logger.debug("RegionMonitoringLocationProvider: Creating delegate assignment for CLLocationManager")
    ...
    func addRegion(center: CLLocationCoordinate2D, radius: CLLocationDistance, identifier: String) {
        ...
        if authorizationStatus == .authorizedWhenInUse || authorizationStatus == .authorizedAlways {
            ...
            logger.debug("RegionMonitoringLocationProvider: Current monitored regions before adding: \(formatRegionDetails(locationManager.monitoredRegions))")
            locationManager.startMonitoring(for: region)
            logger.debug("RegionMonitoringLocationProvider: Current monitored regions after adding: \(formatRegionDetails(locationManager.monitoredRegions))")
        }
    }
    ...
    func removeRegion(identifier: String) {
        if let region = monitoredRegions.first(where: { $0.identifier == identifier }) {
            ...
            logger.debug("RegionMonitoringLocationProvider: Current monitored regions before removing: \(formatRegionDetails(locationManager.monitoredRegions))")
            monitoredRegions.remove(region)
            locationManager.stopMonitoring(for: region)
            logger.debug("RegionMonitoringLocationProvider: Current monitored regions after removing: \(formatRegionDetails(locationManager.monitoredRegions))")
        }
    }
    ...
    func removeAllRegions() {
        ...
        logger.debug("RegionMonitoringLocationProvider: All system regions before cleanup: \(formatRegionDetails(allSystemRegions))")
        logger.debug("RegionMonitoringLocationProvider: System GPS regions to remove: \(systemGPSRegions.map { $0.identifier })")
...
extension RegionMonitoringLocationProvider {
    ...
    func updateRegions(from rules: [Rule]) {
        ...
        logger.debug("RegionMonitoringLocationProvider: Updating regions from \(rules.count) rules -> \(gpsRules.count) GPS rules")
